...
OUT_CSV = ROOT_DIR / "results" / "policy_sweep_results.csv"

TRACE_FILES = [
    "trace_rd.txt",   # random
    "trace_str.txt",  # stride
    "trace_sq.txt",   # sequential
]
...
def run_sim(trace_file: str, l1_size: int, l1_assoc: int, policy: str) -> str:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Run failed for trace={trace_file}, L1={l1_size}, assoc={l1_assoc}, policy={policy}\n"
            f"STDERR:\n{result.stderr}\n"
            f"STDOUT:\n{result.stdout}"
        )
...
def main():
    ...
    total = len(TRACE_FILES) * len(L1_SIZES) * len(L1_ASSOCS) * len(POLICIES)
    done = 0

    for trace_file, l1_size, l1_assoc, policy in itertools.product(
        TRACE_FILES, L1_SIZES, L1_ASSOCS, POLICIES
    ):
        done += 1
        print(f"[{done}/{total}] Running trace={trace_file}, L1={l1_size}, assoc={l1_assoc}, policy={policy}")
        stdout = run_sim(trace_file, l1_size, l1_assoc, policy)
